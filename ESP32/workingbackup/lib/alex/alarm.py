...
class AlarmDaily:
    ...
    def add_alarm(self, hour, minute, second, callback):
        self.alarms.append({'time': (hour, minute, second), 'action': callback})

    def check_alarms(self):
        now = self.rtc.datetime()
        # if now[6] == 0: # Check at the start of the minute
        for alarm in self.alarms:
            if now[4] == alarm['time'][0] and now[5] == alarm['time'][1] and now[6] == alarm['time'][2]:
                alarm['action']()
...
class AlarmWeekly:
    ...
    def add_weekly_alarm(self, days, hour, minute, second, callback):
        ...
        self.alarms.append({'days': days, 'time': (hour, minute, second), 'action': callback})
    ...
    def check_alarms(self):
        ...
        for alarm in self.alarms:
            ...
            if weekday in alarm['days'] and hour == al_h and minute == al_m and second == al_s:
                alarm['action']()
                time.sleep(1) # Block for 1s to prevent re-triggering
